...
//----------------------------------------------------------------------------------
// Module Functions Definition0
//----------------------------------------------------------------------------------
GuiFileDialogState InitGuiFileDialog(int width, int height, const char *initPath, bool active = true)
{
    assert(width <= GetScreenWidth() && height <= GetScreenHeight());
    GuiFileDialogState state = { 0 };

    // Init window data
    state.windowBounds = (Rectangle){ (GetScreenWidth() - width) / 2.0f, (GetScreenHeight() - height) / 2.0f, (float)width, (float)height };
...
# endif
        ...
        Vector2 textSize = MeasureTextEx(GuiGetFont(), "File filter:", GuiGetStyle(DEFAULT, TEXT_SIZE), GuiGetStyle(DEFAULT, TEXT_SPACING));
        if (GuiTextBox((Rectangle){ state->windowBounds.x + 8 + textSize.x + 8, state->windowBounds.y + state->windowBounds.height - 68, state->windowBounds.width - 184 - textSize.x - 16 + 72, 24 }, state->fileNameText, 128, state->fileNameEditMode))
        {
        ...
        textSize = MeasureTextEx(GuiGetFont(), "File filter:", GuiGetStyle(DEFAULT, TEXT_SIZE), GuiGetStyle(DEFAULT, TEXT_SPACING));
        state->fileTypeActive = GuiComboBox((Rectangle){ state->windowBounds.x + 8 + textSize.x + 8, state->windowBounds.y + state->windowBounds.height - 24 - 12, state->windowBounds.width - 184 - textSize.x - 16 + 72, 24 }, "All files", state->fileTypeActive);
