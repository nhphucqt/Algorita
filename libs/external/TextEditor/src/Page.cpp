...
void Page::Draw(Rectangle bounds)
{
	...
	DrawRectangle(bounds.x, bounds.y, 20, bounds.height, GRAY);
	for (int i = 0; i < m_Lines.size(); ++i)
	{
		...
		DrawTextEx(*Gfont::defaultFont, lineNumber, {bounds.x + 15 - width, bounds.y + 10 + i * 20}, 30, 0, BLACK);
	}
	...
	for (int i = 0; i < (int)m_Lines.size(); ++i)
	{
		...
		DrawTextEx(*Gfont::defaultFont, text.c_str(), {bounds.x + 20, bounds.y + 10 + i * 20}, 30, 0, BLACK);
	}
}
