#include "Screen234Tree.h"

Screen::Screen234Tree::Screen234Tree() {}
Screen::Screen234Tree::~Screen234Tree() {}

void Screen::Screen234Tree::load() { // Ensure that obj has to be destroy before (prevent MEMORY LEAKING)
...
void Screen::Screen234Tree::init() {
    currOperationType = OTNULL;
    currOperation = ONULL;
    exitMessage.assign(obj.initialize(GetRandomValue(1, Core::MAX_NUM_234_ELM), &ALOG).message);
}

void Screen::Screen234Tree::draw() {
    bool keyActive = true;
    if (toggleCreateType.draw()) {
        ...
        if (toggleCreateRandom.draw()) {
            ...
            if (GuiButton(Rectangle{(Gui::BUTTON_OPER_WIDTH + Gui::BUTTON_OPER_DIST_X) * 2 + Gui::BUTTON_OPER_WIDTH - (Gui::BUTTON_OPER_GO_WIDTH + 5) * 2, Window::HEIGHT - Layout::BOTTOM_HEIGHT - Gui::BUTTON_OPER_HEIGHT * 4 + 5, Gui::BUTTON_OPER_GO_WIDTH, Gui::BUTTON_OPER_GO_HEIGHT}, Gicon::BUTTON_RANDOM)) {
                inputCreateRandomSize.setnum(GetRandomValue(0, Core::MAX_NUM_234_ELM));
            }
...
void Screen::Screen234Tree::destroy() {
    ALOG.destroy();
    obj.destroy();
}