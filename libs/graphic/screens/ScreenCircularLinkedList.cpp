...
Screen::ScreenCircularLinkedList::ScreenCircularLinkedList() {}

void Screen::ScreenCircularLinkedList::load() { // Ensure that obj has to be destroy before (prevent MEMORY LEAKING)
    ...
    if (toggleCreateType.draw()) {
        ...
        if (GuiButton(Rectangle{(Gui::BUTTON_OPER_WIDTH + Gui::BUTTON_OPER_DIST_X) * 3 + Gui::USER_DEF_BUTTON_WIDTH + Gui::BUTTON_OPER_DIST_X, Window::HEIGHT - Layout::BOTTOM_HEIGHT - Gui::BUTTON_OPER_HEIGHT * 6, Gui::FILE_DIALOG_OPEN_BUTTON_WIDTH, Gui::FILE_DIALOG_OPEN_BUTTON_HEIGHT}, "File")) {
            ...
            if (filePath != nullptr) {
                ...
                if (!fin.is_open()) {
                    std::cerr << "ERRO: Cannot open " << filePath << '\n';
                } else {
                    std::stringstream ss;
                    ss << fin.rdbuf();
                    fin.close();
...
void Screen::ScreenCircularLinkedList::destroy() {
    ALOG.destroy();
    obj.destroy();
}